...
const Player = () => {
  ...
  const { courseID } = useParams(); // Use courseID to match the route parameter
  ...
  // Fetch course data
  const getCourseData = () => {
    const course = enrolledCourses.find((course) => course._id === courseID); // Use courseID here
  ...
  // Fetch course data on mount & when enrolledCourses updates
  useEffect(() => {
    getCourseData();
  }, [enrolledCourses, courseID]); // Add courseID as a dependency
  ...
  if (!courseID) {
    return <div>Course ID is missing. Please check the URL.</div>; // Fallback UI
  }
